...
# Constants
# pigpio has 255 steps of brightness: pi.set_PWM_dutycycle(pin, 255) corresponds to full brightness
RANGE = 255
...
START = {  # Starting point (time) of each color in [0, 1]
    "red": 0,
    "green": 1/3,
    "blue": 2/3,
}
END = {  # Ending point (time) of each color in [0, 1]
    "red": 1,
    "green": 1,
    "blue": 1,
}
# Maximum intensity of the lights (lower if stuff gets too hot)
MAX_INTENSITY = 0.6
...
def sunrise(duration_min, hold_min):
    ...
    for step in range(STEPS + 1):
        ...
        for color in ["red", "green", "blue"]:
            color_progress = bound(0, 1, scaleFctToInt(
                START[color], END[color], GROWTH_FUNC, progress))
            cmd = RANGE * color_progress * MAX_INTENSITY
            pi.set_PWM_dutycycle(PINS[color], cmd)
            print(
                f'{color[0].capitalize()}:{color_progress * 100:3.0f}% ({cmd:3.0f}) ', end='')
        print('')

        # Sleep to reach the overall duration
        time.sleep(interval_length_s)

    # Remain at full light
    print(f'Remain at full light for {hold_min} min')
...
if __name__ == "__main__":
    ...
    hold_min = float(sys.argv[3])
    try:
        if func == "sunrise":
            sunrise(duration, hold_min)
